define(["require", "exports", "VSS/Controls", "VSS/Controls/Grids", "VSS/Utils/Date", "VSS/Utils/Number", "TFS/Work/RestClient", "VSS/Service", "TFS/WorkItemTracking/RestClient"], function (require, exports, Controls, Grids, Date_Utils, Number_Utils, TFS_Work_WebApi, VSS_Service, TFS_Wit_WebApi) {
    "use strict";
    ...
    witClient.queryByWiql(query, projectId).then(function (result) {
        ...
        witClient.getWorkItems(openWorkItems, fields).then(function (workItems) {
            var div = document.getElementById("projectId");
            div.textContent = 'Effort total: ' + workItems.reduce(function (acc, wi) { return acc; });
        });
    });
    ...
    function getColumns() {
        return [
            ...
            },
            {
                ...
                width: 150,
                getCellContents: function (rowInfo, dataIndex, expandedState, level, column, indentIndex, columnOrder) {
                    var unit = this.getColumnValue(dataIndex, "unit") || 0, cost = this.getColumnValue(dataIndex, "cost") || 0, total = unit * cost;
                ...
                comparer: function (column, order, item1, item2) {
                    var total1 = (item1.unit || 0) * (item1.cost || 0), total2 = (item2.unit || 0) * (item2.cost || 0);
